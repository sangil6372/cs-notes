...
닫는 `**` **앞이 구두점이고 뒤가 한글**이면 CommonMark가 닫기로 인정하지 않아 별표가 그대로 보입니다.

```mdx
이게 **TTL(Time To Live)**이다.          <!-- 깨짐: ) 다음 ** 다음 한글 -->
이게 <strong>TTL(Time To Live)</strong>이다.   <!-- 정상 -->
```

뒤에 공백이나 구두점이 오면 문제없습니다(`**기억한다.** 이것이`). 애매하면 `<strong>`을 쓰세요. 아래 명령으로 한 번에 찾을 수 있습니다.

```bash
grep -nE '[)"]\*\*[가-힣]' content/**/*.mdx
```
